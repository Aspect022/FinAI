...
import { Plus, Bus, Receipt, Utensils, Wallet, TrendingUp, TrendingDown } from "lucide-react"
...
export default function HistoryPage() {
  ...
  const iconFor = (category: string, type: string) => {
    if (type === "income") {
      if (/daily|freelance|tips/i.test(category)) return <TrendingUp className="h-4 w-4 text-emerald-600" />
      return <Wallet className="h-4 w-4 text-emerald-600" />
    }
    if (/food/i.test(category)) return <Utensils className="h-4 w-4 text-red-600" />
    if (/transport/i.test(category)) return <Bus className="h-4 w-4 text-red-600" />
    if (/bill/i.test(category)) return <Receipt className="h-4 w-4 text-red-600" />
    ...
    <main className="p-4 md:p-6 space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent className="text-2xl font-semibold">₹{summary.income.toLocaleString("en-IN")}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">{t("monthlyExpenses")}</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">₹{summary.expense.toLocaleString("en-IN")}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">{t("netSavings")}</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">₹{summary.savings.toLocaleString("en-IN")}</CardContent>
        </Card>
      </section>
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {new Date(i.date).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })} ·{" "}
                          {i.notes || i.merchant || "-"}
                        </div>
                      </div>
                    </div>
                    <div
                      className={`text-sm font-semibold ${i.type === "income" ? "text-emerald-600" : "text-red-600"}`}
                    >
                      {i.type === "income" ? "+" : "-"}₹{i.amount.toLocaleString("en-IN")}
                    </div>
                  </div>
                ))}
              </CardContent>
      ...
      <Button onClick={() => setOpen(true)} className="fixed bottom-20 right-5 z-40 rounded-full h-12 px-5 shadow-lg">
